...
class InformationTheoryEnhancer:
        ...
        if target_col_idx is None:
            try:
                ...
                if volatility_indices:
                    ...
                    print(f"Using volatility column at index {target_col_idx}: '{self.feature_names[target_col_idx]}'")
                else:
                    print("WARNING: No volatility column found in feature names. Using last column as target.")
                    target_col_idx = -1  # Default to last column
            except ValueError:
                print("WARNING: Error finding volatility column. Using last column as target.")
                target_col_idx = -1  # Default to last column
    ...
    def compute_transfer_entropy(self, lag=1, k=1):
        ...
        # Print volatility column diagnostics
        print(f"Volatility column index: {self.target_col_idx}")
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.feature_array.shape[1]:
            volatility_data = self.feature_array[:, self.target_col_idx]
            q25, q50, q75 = np.percentile(volatility_data, [25, 50, 75])
            iqr = q75 - q25
            print(f"Volatility column statistics:")
            print(f"  - Name: '{self.feature_names[self.target_col_idx]}'")
            print(f"  - Min: {volatility_data.min():.6f}, Max: {volatility_data.max():.6f}")
            print(f"  - Mean: {volatility_data.mean():.6f}, Median: {q50:.6f}")
            print(f"  - IQR: {iqr:.6f} (Q1: {q25:.6f}, Q3: {q75:.6f})")
            print(f"  - Standard deviation: {volatility_data.std():.6f}")
        else:
            print("WARNING: No valid volatility column found to analyze")
    ...
    def select_top_features(self, n_features=10, min_score=0.1):
        ...
        if self.feature_importance is None:
            ...
            if self.mi_matrix is not None:
                ...
                # Ensure we include volatility column if it exists
                if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
                    selected = list(range(min(n_features-1, self.n_features)))
                    if self.target_col_idx not in selected:
                        selected.append(self.target_col_idx)
                    return selected
                else:
        ...
        # Ensure volatility column is always included
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            if self.target_col_idx not in selected_indices:
                print(f"Forcing inclusion of volatility column (index {self.target_col_idx})")
                # Add volatility column and remove the lowest scoring feature if necessary
                if len(selected_indices) >= n_features:
                    selected_indices.pop()  # Remove the lowest scoring feature
                selected_indices.append(self.target_col_idx)
    ...
    def enhance_features(self, feature_array, feature_names, n_clusters=10, use_entropy=True, 
        ...
        # Get original volatility feature index and value if it exists
        volatility_idx = None
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            volatility_idx = self.target_col_idx
            volatility_name = self.feature_names[volatility_idx]
            volatility_data = self.feature_array[:, volatility_idx]
            print(f"Preserving volatility column '{volatility_name}' (index {volatility_idx}) during enhancement")
        ...
        # Ensure volatility is in the top features if it exists
        if volatility_idx is not None and volatility_idx not in top_feature_indices:
            # Remove the lowest scoring feature
            top_feature_indices = top_feature_indices[1:]
            # Add volatility
            top_feature_indices = np.append(top_feature_indices, volatility_idx)
            print(f"Added volatility column '{volatility_name}' to top features")
        ...
        # Final check: ensure volatility is still in the enhanced features
        if volatility_idx is not None:
            volatility_in_enhanced = False
            for i, name in enumerate(enhanced_feature_names):
                if name == volatility_name:
                    volatility_in_enhanced = True
                    print(f"Confirmed volatility column '{volatility_name}' is present in enhanced features at index {i}")
                    break
            
            if not volatility_in_enhanced:
                print(f"Volatility column '{volatility_name}' was lost during enhancement - adding it back")
                # Add the volatility column to the enhanced features
                enhanced_features = np.column_stack((enhanced_features, volatility_data))
                enhanced_feature_names.append(volatility_name)
                print(f"Added volatility column '{volatility_name}' to enhanced features at index {len(enhanced_feature_names)-1}")
    ...
    def enhance_features_adaptive(self, n_features=10, is_training=True, 
        ...
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            ...
            volatility_name = self.feature_names[volatility_idx]
            volatility_data = self.feature_array[:, volatility_idx]
            print(f"Preserving volatility column '{volatility_name}' (index {volatility_idx}) during adaptive enhancement")
        ...
        # Make sure the selector knows about the volatility target column
        selector.target_col_idx = self.target_col_idx
        ...
        # Ensure volatility column is included in the selected indices
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            if self.target_col_idx not in selected_indices:
                print(f"Adding volatility column (index {self.target_col_idx}) to selected features")
                selected_indices.append(self.target_col_idx)
                
                # Add characteristics for the volatility feature if not already present
                if self.target_col_idx not in characteristics:
                    characteristics[self.target_col_idx] = {
                        'name': self.feature_names[self.target_col_idx],
                        'relevance': 1.0,  # Force high relevance
                        'entropy': self.entropy[self.target_col_idx] if self.entropy is not None else 0,
                        'snr': 1.0,  # Force high SNR
                        'heavy_tailed': False,
                        'te_importance': 1.0  # Force high TE importance
                    }
        ...
        # Print enhanced feature shape and names for diagnostic purposes
        print(f"Enhanced features shape: {enhanced_features.shape}")
        print(f"Enhanced feature names: {enhanced_names}")
        
        # Check if volatility feature is in enhanced features
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            volatility_name = self.feature_names[self.target_col_idx]
            if volatility_name in enhanced_names:
                print(f"Found volatility feature in enhanced features: {volatility_name}")
            else:
                print(f"WARNING: Volatility feature '{volatility_name}' is missing from enhanced features")
                for vol_like in [name for name in enhanced_names if 'vol' in name.lower()]:
                    print(f"  - Possible volatility-related feature found: {vol_like}")
        ...
        # Final check: ensure volatility is still in the enhanced features
        if volatility_idx is not None and volatility_name is not None and volatility_data is not None:
            volatility_in_enhanced = False
            ...
            if not volatility_in_enhanced:
                print(f"Volatility column '{volatility_name}' was lost during adaptive enhancement - adding it back")
                # Add the volatility column to the enhanced features
                enhanced_features = np.column_stack([enhanced_features, volatility_data])
                enhanced_names.append(volatility_name)
                print(f"Added volatility column '{volatility_name}' to enhanced features at index {len(enhanced_names)-1}")
    ...
    def enhance_features_batch(self, n_features=10, include_entropy=True):
        ...
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            ...
            volatility_name = self.feature_names[volatility_idx]
            volatility_data = self.feature_array[:, volatility_idx]
            print(f"Preserving volatility column '{volatility_name}' (index {volatility_idx}) during batch enhancement")
        ...
        # Make sure the selector knows about the volatility column
        if volatility_idx is not None:
            selector.target_col_idx = volatility_idx
        ...
        # Ensure volatility is included
        if volatility_idx is not None and volatility_idx not in selected_indices:
            print(f"Adding volatility column '{volatility_name}' to batch-selected features")
            if len(selected_indices) >= n_features:
                # Remove the last (lowest importance) feature
                selected_indices = selected_indices[:-1]
            selected_indices = np.append(selected_indices, volatility_idx)
        ...
        if volatility_idx is not None and volatility_name is not None and volatility_data is not None:
            ...
            for i, name in enumerate(enhanced_names):
                if name == volatility_name:
                    volatility_in_enhanced = True
                    print(f"Confirmed volatility column '{volatility_name}' is present in batch-enhanced features at index {i}")
                    break
            
            if not volatility_in_enhanced:
                print(f"Volatility column '{volatility_name}' was lost during batch enhancement - adding it back")
                # Add the volatility column to the enhanced features
                enhanced_features = np.column_stack([enhanced_features, volatility_data])
                enhanced_names.append(volatility_name)
                print(f"Added volatility column '{volatility_name}' to enhanced features at index {len(enhanced_names)-1}")
...
class ProgressiveFeatureSelector:
    ...
    def final_feature_selection(self, n_features=10, refine_with_te=True):
        ...
        # Make sure volatility column is in the candidates
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            if self.target_col_idx not in candidates:
                print(f"Adding volatility column (index {self.target_col_idx}) to candidate features")
                candidates.append(self.target_col_idx)
        ...
        while len(selected) < n_features and remaining:
            ...
            for idx in remaining:
                ...
                for sel_idx in selected:
                    ...
                    idx_candidate_idx = np.where(candidates == sel_idx)[0][0]
        ...
        # Make sure volatility column is in the final selection
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            if self.target_col_idx not in selected:
                print(f"Forcing inclusion of volatility column (index {self.target_col_idx}) in final selection")
                # Remove lowest scoring feature if we're at the limit
                if len(selected) >= n_features:
                    # Find feature with lowest relevance score
                    min_relevance = float('inf')
                    min_idx = None
                    for idx in selected:
                        if idx in self.relevance_scores and self.relevance_scores[idx] < min_relevance:
                            min_relevance = self.relevance_scores[idx]
                            min_idx = idx
                    
                    # Remove feature with lowest relevance
                    if min_idx is not None:
                        selected.remove(min_idx)
                        print(f"Removed feature {self.feature_names[min_idx]} (index {min_idx}) with relevance {min_relevance:.4f}")
                    else:
                        # If we can't find by score, remove the last one
                        removed_idx = selected.pop()
                        print(f"Removed feature {self.feature_names[removed_idx]} (index {removed_idx})")
                
                # Add volatility column
                selected.append(self.target_col_idx)
                
                # Add characteristics for volatility if not exists
                if self.target_col_idx not in characteristics:
                    characteristics[self.target_col_idx] = {
                        'name': self.feature_names[self.target_col_idx],
                        'relevance': 1.0,  # Force high relevance
                        ...
                        'snr': 1.0,        # Force high SNR
                        'heavy_tailed': False,
                        'te_importance': 1.0  # Force high transfer entropy importance
                    }
    ...
    def fast_feature_selection(self, n_features=10, include_redundancy=False):
        ...
        # Always include the volatility column if it exists
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            # Ensure volatility has a high score to be selected
            nmi[self.target_col_idx] = max(nmi) * 1.1  # Set higher than the maximum score
        ...
        # Final check to make sure volatility is included
        if self.target_col_idx is not None and 0 <= self.target_col_idx < self.n_features:
            if self.target_col_idx not in selected:
                print(f"Forcing inclusion of volatility column (index {self.target_col_idx}) in fast selection")
                # Remove the lowest scoring feature if we're at capacity
                if len(selected) >= n_features:
                    # Find position of lowest scoring feature
                    lowest_idx = np.argmin([nmi[idx] for idx in selected])
                    print(f"Removing feature at index {selected[lowest_idx]} with score {nmi[selected[lowest_idx]]:.4f}")
                    selected = np.delete(selected, lowest_idx)
                
                # Add volatility column
                selected = np.append(selected, self.target_col_idx)
        ...
        while len(selected) < n_features and remaining:
            ...
            for idx in remaining:
                ...
                for sel_idx in selected:
                    ...
                    sel_candidate_idx = candidate_indices.index(sel_idx)
                    idx_candidate_idx = candidate_indices.index(idx)
                    redundancy_penalty += redundancy[sel_candidate_idx, idx_candidate_idx]
